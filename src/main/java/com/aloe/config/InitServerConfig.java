...
public class InitServerConfig implements SmartLifecycle {
	...
	public void stop(final Runnable callback) {
		logger.info("---------------->服务器停止->stop(callback)");
		/**如果你让isRunning返回true，需要执行stop这个方法，那么就不要忘记调用callback.run()。
		 * 否则在你程序退出时，Spring的DefaultLifecycleProcessor会认为你这个TestSmartLifecycle没有stop完成，
		 * 程序会一直卡着结束不了，等待一定时间（默认超时时间30秒）后才会自动结束。
		 * 如果你想修改这个默认超时时间 ，设置org.springframework.context.support.DefaultLifecycleProcessor timeoutPerShutdownPhase
		 */
       callback.run();
		isRunning = false;
	}
