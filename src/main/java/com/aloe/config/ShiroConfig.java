...
public class ShiroConfig {
    ...
    public CookieRememberMeManager rememberMeManager() {
        logger.info("cookie管理对象->ShiroConfiguration.rememberMeManager()");
    ...
    public SecurityManager securityManager() {
        logger.info("安全验证管理器-->securityManager", ShiroFilterFactoryBean.class);
    ...
    public ServletContextInitializer servletContextInitializer() {
        return new ServletContextInitializer() {
            ...
            public void onStartup(ServletContext servletContext) throws ServletException {
                logger.info("servlet初始化：servletContextInitializer.onStartup");
                //只用cookie
                servletContext.setSessionTrackingModes(Collections.singleton(SessionTrackingMode.COOKIE));
                SessionCookieConfig sessionCookieConfig = servletContext.getSessionCookieConfig();
                // cookie 只能通过http传输使用 解决 jsessionid 问题
